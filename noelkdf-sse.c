...
static inline uint32_t hashBlocks(uint32_t value, uint32_t *mem, uint32_t blocklen, uint64_t fromAddr,
    ...
    __m128i *prev = (__m128i *)(mem + prevAddr);
    __m128i *to = (__m128i *)(mem + toAddr);
    __m128i *from = (__m128i *)(mem + fromAddr);
    __m128i v = _mm_set_epi32(value, value, value, value);
    __m128i const3 = _mm_set_epi32(3, 3, 3, 3);
    uint32_t i;
    uint32_t r;
    for(r = 0; r < repetitions; r++) {
        for(i = 0; i < blocklen/4; i++) {
            __m128i p = _mm_load_si128(prev++);
            __m128i f = _mm_load_si128(from++);
            __m128i pOr3 = _mm_or_si128(p, const3);
            __m128i multRes = _mm_mullo_epi32 (v, pOr3);
            v = _mm_add_epi32(multRes, f);
            _mm_store_si128(to++, v);
        }
    }
    for(i = 0; i < 4; i++) {
        uint32_t val = _mm_extract_epi32 (v, i);
        value ^= val;
    }
    return value;
    /*
    ...
    uint64_t prevAddr = toAddr - blocklen;
    uint32_t *prev = mem + prevAddr;
    uint32_t *to = mem + toAddr;
    uint32_t *from = mem + fromAddr;
    uint32_t v[parallelism];
    uint32_t i;
    uint32_t r;
    for(r = 0; r < repetitions; r++) {
        for(i = 0; i < blocklen/parallelism; i++) {
            uint32_t j;
            for(j = 0; j < parallelism; j++) {
                v[j] = v[j]*((*prev++ << 2) | 3) + *from++;
                *to++ = v[j];
            }
        }
    }
